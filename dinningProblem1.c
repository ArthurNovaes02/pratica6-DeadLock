...
void * philosopher (void *num)
{
	...
	while (f) {
		// pega os chopstrick
		if (pthread_mutex_trylock (&chopstick[left_chopstick]) == 0) {
			// printf ("Philosopher %ld: got left chopstick %ld\n", id,  left_chopstick);
			// pega a segunda vareta
			if (pthread_mutex_trylock (&chopstick[right_chopstick]) == 0) {
				...
				// a outra vareta esta ocupada
				// printf ("Philosopher %ld: drop left chopstick %ld\n", id,  left_chopstick);
				pthread_mutex_unlock (&chopstick[left_chopstick]);			
			}
		}
	}
